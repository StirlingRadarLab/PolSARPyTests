...
# OPENING XARRAY 
ds = xr.open_zarr(path_Py_short/"h_a_alpha_decomposition.zarr")
...
array_file = list(ds.data_vars)

ds[array_file[0]].shape
col = ds[array_file[0]].shape[0]
row = ds[array_file[0]].shape[1]

imgPy  = np.zeros([col, row, num_dim])

for i in range(num_dim):
    imgPy[:,:,i] = ds[array_file[i]].values
...
for i in range(num_pro):
    ...
    for ii in range(num_file):
        ...
        img_temp = Open_ENVI_Image(path_PSP / (array_file[ii] + '.bin'), col, row, dtypePSP)
...
# this may crush when running all together and it is suggested to have iii_select = []
    ...
    for iii in iii_select: 
        ...
        im = plt.imshow(np.abs(imgPy[:,:,iii]), cmap = colormap[iii], vmin=scale[0], vmax=scale[1])
        plt.colorbar(im, fraction=0.046, pad=0.04)
        plt.savefig(path_save / ('PSP_' + array_file[iii]), bbox_inches='tight', pad_inches=0)
    ...
    for ii in range(num_file):
        ...
        plt.savefig(path_save / ('Histogram_' + str(array_file[ii]) + '_Python'))
        ...
        plt.plot(val[:-1], hist[0])
        plt.axis([np.min(hist[1]), np.max(hist[1]), 0, np.max(hist[0])])
        plt.savefig(path_save / ('Histogram_' + str(array_file[ii]) + '_POLSARpro'))
        ...
        plt.savefig(path_save / ('Histogram_DIFFERENCE_' + str(array_file[ii]) + '_Python'))      
...
#%% Mean difference
    ...
    for ii in range(num_file):
        ...
        mean.update( {str(array_file[ii])+' PSP mean': [imgPSP_mean[ii]], 
