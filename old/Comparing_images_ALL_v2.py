...
path_PPP_short = Path("/home/am221/C/Data/PolSARPy/Output_PPP")
...
ds = xr.open_zarr(path_Py_short/"HAAlpha.zarr")
...
for i in range(num_pro):
    ...
    # get the filenames
    array_file_ext = os.listdir(path_Py_short + array_pro[i])
    # remove the config file if present
    if 'config.txt' in array_file_ext: array_file_ext.remove(array_file_ext[0])
    
    array_file = [f for f in array_file_ext if f.endswith('.img')]
    array_file = [f.replace('.img', '') for f in array_file]
    array_file = [f.replace('_py', '') for f in array_file]
    ...
    for ii in range(num_file):
        ...
        imgPy[:,:,ii]  = Open_ENVI_Image(path_Py  + array_file[ii] + '_py.img', col, row, dtypePy) 
    ...
    # plorting the RGB for the elements
    plot_RGB(imgPPP[:,:,1], imgPPP[:,:,2], imgPPP[:,:,0], 
             str(array_pro[i]) + ' RGB: PolSARpro', fact = 1.5, 
             path_out =path_save + 'RGB_PPP')
...
# this may crush when running all together and it is suggested to have iii_select = []
    ...
    for iii in iii_select: 
        ...
        plt.title('Python ' + str(array_pro[i]) + ': ' + array_file[iii]) 
        scale2 = (0, np.abs(imgPy[:,:,0]).mean()*2)
        plt.imshow(np.abs(imgPy[:,:,iii]), cmap = colormap[iii], vmin=scale[0], vmax=scale[1])
        plt.savefig(path_save + 'Py_' + array_file[iii], bbox_inches='tight', pad_inches=0)


        fig = plt.figure(figsize=(screen_width / dpi, screen_height / dpi), dpi=dpi)    
        scale = (0, np.abs(imgPPP[:,:,iii]).mean()*2)
        plt.title('PolSARPro ' + str(array_pro[i]) + ': ' + array_file[iii]) 
        scale2 = (0, np.abs(imgPPP[:,:,0]).mean()*2)
        plt.imshow(np.abs(imgPy[:,:,iii]), cmap = colormap[iii], vmin=scale[0], vmax=scale[1])
        plt.savefig(path_save + 'PPP_' + array_file[iii], bbox_inches='tight', pad_inches=0)
    ...
    #      np.transpose(Diff[:,:,2]), np.transpose(imgPPP[:,:,2]), 
    ...
    #      scale4 = [0, np.abs(imgPPP[:,:,2]).mean()*2],
    #      path_out = path_save + 'Difference_plot') 



    imgPPP_mean = np.zeros([num_file])
    ...
    for ii in range(num_file):
        ...
        val = hist[1]
        fig = plt.figure(figsize=(screen_width / dpi, screen_height / dpi), dpi=dpi)    
        plt.title('Histogram of ' + str(array_file[ii]) + ' Python')
        plt.plot(val[:-1], hist[0])
        plt.axis([np.min(hist[1]), np.max(hist[1]), 0, min_freq])
        plt.savefig(path_save + 'Histogram of ' + str(array_file[ii]) + ' Python')
    
        PPP1D = np.ravel(imgPPP[:,:,ii])
        hist = np.histogram(PPP1D, bins=100)
        val = hist[1]
        fig = plt.figure(figsize=(screen_width / dpi, screen_height / dpi), dpi=dpi)    
        plt.title('Histogram of ' + str(array_file[ii]) + ' POLSARpro')
        plt.plot(val[:-1], hist[0])
        plt.axis([np.min(hist[1]), np.max(hist[1]), 0, min_freq])
        plt.savefig(path_save + 'Histogram of ' + str(array_file[ii]) + ' POLSARpro')
        ...
        val = hist[1]
        fig = plt.figure(figsize=(screen_width / dpi, screen_height / dpi), dpi=dpi)    
        plt.title('Histogram of DIFFERENCE ' + str(array_file[ii]) + ' Python')
        plt.plot(val[:-1], hist[0])
        plt.axis([np.min(hist[1]), np.max(hist[1]), 0, min_freq])
        plt.savefig(path_save + 'Histogram of DIFFERENCE ' + str(array_file[ii]) + ' Python')      


#%% Mean difference
    ##################################################################

        imgPPP_mean[ii] = np.mean(np.abs(imgPPP[:,:, ii]))
        imgPy_mean[ii]  = np.mean(np.abs(imgPy[:,:, ii]))
    
        Diff_mean[ii] = np.mean(np.abs(Diff[:,:,ii]))
    
        RMSE_mean[ii] = np.sqrt(np.mean( (imgPPP[:,:,ii] - imgPy[:,:,ii])**2 ) )
    
        Norm_RMSE_mean[ii] = np.sqrt(np.mean( (imgPPP[:,:,ii] - imgPy[:,:,ii])**2 / (imgPPP[:,:,ii] + imgPy[:,:,ii])**2 ) )
    
        Norm_diff_mean[ii] = np.mean( (imgPPP[:,:,ii] - imgPy[:,:,ii]) / (imgPPP[:,:,ii] + imgPy[:,:,ii]) ) 
    
        Norm_abs_diff_mean[ii] = np.mean( np.abs(imgPPP[:,:,ii] - imgPy[:,:,ii]) / (imgPPP[:,:,ii] + imgPy[:,:,ii]) )


    # # Normalising the error
    # Norm_Ground_RMSE = Ground_RMSE/np.mean(GroundPPP) 
    # Norm_Volume_RMSE = Volume_RMSE/np.mean(VolumePPP) 
    ...
    for ii in range(num_file):
        ...
        xaxis_array = [str(array_file[ii])+' RMSE', 
                       str(array_file[ii])+' NDI',
                       ]
    ...
    fig.savefig(path_save + str(array_pro[i]) + ' Mean of Differences')          
    ...
    for ii in range(num_file):
        ...
        xaxis_array = np.append(xaxis_array, [str(array_file[ii])+' 50th', str(array_file[ii])+' 90th', str(array_file[ii])+' 99th'])
    ...
    fig.savefig(path_save + str(array_pro[i]) + ' Percentiles')          
    ...
    plt.xticks(rotation=45)  # ← rotate x-axis labels by 45 degrees
    fig.savefig(path_save + str(array_pro[i]) + ' Normalised Percentiles (over mean)')     
    ...
    for ii in range(num_file):
        ...
        mean.update( {str(array_file[ii])+' PPP mean': [imgPPP_mean[ii]], 
                      str(array_file[ii])+' Py mean': [imgPy_mean[ii]]
                     } )
        
        diverg.update( {str(array_file[ii])+' Diff mean': [Diff_mean[ii]], 
                        str(array_file[ii])+' RMSE mean': [RMSE_mean[ii]],
                        str(array_file[ii])+' Norm RMSE mean': [Norm_RMSE_mean[ii]],
                        str(array_file[ii])+' Norm diff mean': [Norm_diff_mean[ii]],
                        str(array_file[ii])+' Norm abs diff mean': [Norm_abs_diff_mean[ii]]
                   } )
        
        perc.update( {str(array_file[ii])+' 50th': [Diff_Perc50[ii]], 
                      str(array_file[ii])+' 90th': [Diff_Perc90[ii]],
                      str(array_file[ii])+' 99th': [Diff_Perc99[ii]]
                     } )

        perc_norm.update( {str(array_file[ii])+' 50th': [Diff_Perc50[ii]/imgPy_mean[ii]], 
                           str(array_file[ii])+' 90th': [Diff_Perc90[ii]/imgPy_mean[ii]], 
                           str(array_file[ii])+' 99th': [Diff_Perc99[ii]/imgPy_mean[ii]], 
                   } )        
